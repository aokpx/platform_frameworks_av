        ...
        mId(android_atomic_inc(&nextTrackId))
{
    ...
    if ((int16_t)flags == 0x1) {
       ...
       {
          bufferSize = frameCount*mChannelCount*32; // full rate frame size
       }
       else if (format == AUDIO_FORMAT_EVRC)
       {
          bufferSize = frameCount*mChannelCount*23; // full rate frame size
       }
       else if (format == AUDIO_FORMAT_QCELP)
       {
          bufferSize = frameCount*mChannelCount*35; // full rate frame size
       }
       ...
       {
          bufferSize = frameCount*mChannelCount*61; // full rate frame size
       }
    }
#else
...
#ifdef QCOM_ENHANCED_AUDIO
            if ((int16_t)flags == 0x1) {
                //XXX: copy/paste error from above?
                bufferSize = frameCount*mChannelCount*sizeof(int16_t);
            } else {
               if ((format == AUDIO_FORMAT_PCM_16_BIT) ||
                   (format == AUDIO_FORMAT_PCM_8_BIT))
               {
                  memset(mBuffer, 0, frameCount*mChannelCount*sizeof(int16_t));
               }
               else if (format == AUDIO_FORMAT_AMR_NB)
               {
                  memset(mBuffer, 0, frameCount*mChannelCount*32); // full rate frame size
               }
               else if (format == AUDIO_FORMAT_EVRC)
               {
                  memset(mBuffer, 0, frameCount*mChannelCount*23); // full rate frame size
               }
               else if (format == AUDIO_FORMAT_QCELP)
               {
                  memset(mBuffer, 0, frameCount*mChannelCount*35); // full rate frame size
               }
               else if (format == AUDIO_FORMAT_AAC)
               {
               ...
               {
                  memset(mBuffer, 0, frameCount*mChannelCount*61); // full rate frame size
               }
            }
#else
...
{
    if (mCblk != NULL) {
        //XXX: missing mCblk->frameSize for AudioFlinger.cpp hunk "@@ -4341,7 +4903,12 @@"
...
AudioFlinger::RecordThread::RecordTrack::RecordTrack(
    ...
    :   TrackBase(thread, client, sampleRate, format,
        ...
        mOverflow(false)
{
    //XXX: missing mCblk->frameSize for AudioFlinger.cpp hunk "@@ -5372,12 +5939,44 @@"
